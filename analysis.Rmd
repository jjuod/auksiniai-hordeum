...
```{r hi_impact}
ann_hi = separate_rows(ann_chr5, ANN, sep=",") %>%
  filter(grepl("HIGH", ANN))
nrow(ann_hi)
...
ggplot(ann_hi, aes(x=POS/1e6, y=DP4, col=MAF==1)) +
  geom_point() + theme_bw()
ann_hi = separate(ann_hi, ANN,
                  c("Allele", "Effect", "Impact", "GeneName", "GeneID", "FeatureType", "FeatureID", 
                    ...
                    "Distance", "Errors"), sep="\\|")
ann_hi = select(ann_hi, -one_of(c("GeneID", "FeatureType", "FeatureID", "TranscrBiotype",
                                  "Distance", "conseq", "reliable", "Allele", "Impact")))

biomart = fread("../refs/biomart_go_annots.txt")
ann_hi_go = select(biomart, -one_of(c("Transcript stable ID", "GO term definition"))) %>%
