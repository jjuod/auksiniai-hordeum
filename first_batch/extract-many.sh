...
# And a gene list:
INLIST="auxin_biosynthesis_genes_edited.csv"
# NOTE: one of the genes is missing (HORVU6Hr1G0816706) - it is also 1 char longer than all others
# so replaced w/ HORVU6Hr1G081670
...
# NOTE:
# genes 60-62 are on "Un" chromosome = pieces unassigned to contigs
# so we don't have variant calls for those

# attach line numbers to create output metadata
awk -v OFS="\t" 'BEGIN{print "chr	start	end	strand	type	id	geneName	geneId	numberOfTranscripts	canonicalTranscriptLength	transcriptId	cdsLength	numberOfExons	exonRank	exonSpliceType"} {print NR, $0}' snpeff_onlygenes.txt > extracted_genes.txt

# zip output fastas
zip -r extracted-62.zip extracted/
